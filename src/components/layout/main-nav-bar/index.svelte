...
<div class="nav">
  {#each $dataList$.slice(0, $navBarConfig$.mainNavBarViewCount) as row}
    <RouterLink  bind:this={routerLink} menuName="{row.menuName}" iconData ="{row.iconData}" fontIcon="{row.fontIcon}" useFontIcon={row.useFontIcon} on:navigate={onNavigate} __path={'/' + row.path.replace('/', '--')}  activeClass="active"></RouterLink>
  {/each}
